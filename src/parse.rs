...
impl Parse {
    ...
    pub(crate) fn new(frame: RESP) -> Result<Parse, ParseError> {
    ...
    /// entry is a frame.
    fn next(&mut self) -> Result<RESP, ParseError> {
    ...
    ///
    /// This includes `SimpleString`, `BulkString`, and `Integer` frame types. `SimpleString` and
    /// `BulkString` frame types are parsed.
    ///
...
impl From<String> for ParseError {
    fn from(src: String) -> ParseError {
        ParseError::Other(crate::protocol::Error::Other(src))
    }
}
